	...
	implements Highlighter.HighlightPainter, CaretListener, MouseListener, MouseMotionListener
{
	private JTextComponent component;
        ...
        private int[] highlightsline = new int[50];
	...
	public LinePainter(JTextComponent component, Color color)
	{
		...
		try
		{
			component.getHighlighter().addHighlight(8, 14, this);
		}
	...
	public void setColor(Color color)
	{
        ...
        {
            for( int i = 0; i < this.highlightsline.length ; i++ )
                if( this.highlightsline[i] == -1)
                {
                    this.highlightsline[i] = line;
	...
	public void paint(Graphics g, int p0, int p1, Shape bounds, JTextComponent c)
	{
		try
		{
			...
			g.fillRect(0, r.y, c.getWidth(), r.height);
                        ...
                        g.setColor(ALexico.ALexico_GUI.error_highlight);
                        for( int i = 0 ; i < this.highlightsline.length; i++ )
                            if( this.highlightsline[i] != -1)
                            g.fillRect(0, r.height*this.highlightsline[i]+3,
                                    c.getWidth(), r.height);
